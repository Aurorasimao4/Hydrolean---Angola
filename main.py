...
FastAPI + NaiveBayes + Open-Meteo (Meteo) + DeepSeek API (NLP)
...
Endpoints:
  ...
  POST /chat             → Conversar com o assistente agrícola (DeepSeek)
...
# ============================================================
# DeepSeek — usa o SDK do OpenAI (compatível)
# pip install openai
# ============================================================
...
# Configurar DeepSeek (compatível com OpenAI SDK)
...
def chamar_deepseek(system_prompt: str, user_prompt: str, max_tokens: int = 2000) -> str:
    """Chama a API do DeepSeek via OpenAI SDK."""
...
async def health_check():
    return {
        "status": "online",
        "versao": "2.0.0 — com dados meteorológicos + chat",
        "modelo": "NaiveBayes (GaussianNB)",
        #"culturas_suportadas": len(culturas_labels),
        "deepseek_configurado": deepseek_client is not None,
...
Seja DIRECTO e PRÁTICO."""
    ...
    # 5. Chamar DeepSeek
    resposta_texto = chamar_deepseek(system_prompt, user_prompt)
...
# ============================================================
# CHAT — Conversar com o assistente agrícola (DeepSeek)
# ============================================================
...
class ChatRequest(BaseModel):
    mensagem: str = Field(..., description="Mensagem do utilizador")
    historico: list[ChatMessage] = Field(
        default=[],
        description="Histórico da conversa (para manter contexto multi-turno)",
    )
...
class ChatResponse(BaseModel):
    resposta: str
    historico_atualizado: list[ChatMessage]


CHAT_SYSTEM_PROMPT = """Você é o AgroIntel, um assistente agrícola inteligente especializado 
em irrigação, culturas e agricultura tropical, focado em Angola, Moçambique e Brasil.

SUAS CAPACIDADES:
- Recomendar culturas com base em parâmetros do solo (N, P, K, temperatura, humidade, pH, precipitação)
- Aconselhar sobre irrigação com base em dados meteorológicos em tempo real
- Analisar solo e sugerir correcções
- Dar dicas práticas sobre plantio, colheita e gestão de água
- Ajudar agricultores com linguagem simples e directa

REGRAS:
- Responda SEMPRE em português
- Seja amigável e acessível — muitos agricultores não têm formação técnica
- Dê conselhos PRÁTICOS e APLICÁVEIS
- Se o agricultor fornecer dados do solo, analise e recomende
- Se perguntar sobre irrigação e houver dados meteo disponíveis, use-os
- Pode fazer perguntas para entender melhor a situação do agricultor
- Se não souber algo com certeza, diga honestamente

CULTURAS QUE RECONHECE: Arroz, Milho, Grão-de-bico, Feijão-vermelho, Feijão-guandu, 
Feijão-moth, Feijão-mungo, Feijão-preto, Lentilha, Romã, Banana, Manga, Uva, 
Melancia, Melão, Maçã, Laranja, Papaia, Coco, Algodão, Juta, Café"""


@app.post("/chat", response_model=ChatResponse)
async def chat(req: ChatRequest):
    """
    Conversa com o assistente agrícola AgroIntel (DeepSeek).
    Suporta multi-turno: envia o histórico de volta a cada mensagem.
    """
    if not deepseek_client:
        raise HTTPException(
            status_code=503,
    ...
    # Buscar dados meteo se localização fornecida
    meteo_contexto = ""
    if req.latitude and req.longitude:
        try:
            meteo = await obter_previsao_meteo(req.latitude, req.longitude)
            resumo = meteo["previsao_horaria_resumo"]
            decisao = meteo["decisao_rapida"]
            meteo_contexto = f"""

DADOS METEOROLÓGICOS ACTUAIS ({req.latitude}, {req.longitude}):
- Chuva 6h: {resumo['proximas_6h']['chuva_total_mm']}mm | Temp: {resumo['proximas_6h']['temp_media']}°C
- Chuva 24h: {resumo['proximas_24h']['chuva_total_mm']}mm | Balanço hídrico: {resumo['proximas_24h']['balanco_hidrico_mm']}mm
- Chuva 48h: {resumo['proximas_48h']['chuva_total_mm']}mm
- Decisão irrigação: {"NÃO irrigar" if not decisao['irrigar_agora'] else "Irrigar"} — {decisao['motivo']}"""
        except Exception:
            meteo_contexto = ""

    system_with_meteo = CHAT_SYSTEM_PROMPT + meteo_contexto

    # Montar mensagens para o DeepSeek
    messages = [{"role": "system", "content": system_with_meteo}]

    # Adicionar histórico (máx últimas 20 mensagens para não exceder contexto)
    for msg in req.historico[-20:]:
    ...
    # Atualizar histórico
    historico_atualizado = req.historico.copy()
    historico_atualizado.append(ChatMessage(role="user", content=req.mensagem))
    historico_atualizado.append(ChatMessage(role="assistant", content=resposta))
